...
import { Plus, Wrench, Edit3, Trash2, Loader2, X, AlertCircle } from "lucide-react";
    ...
    <div className="space-y-6 text-stone-900 dark:text-slate-100">
      ...
      {loading ? (
        ...
        <div className="p-12 flex flex-col items-center justify-center gap-2 text-stone-400 dark:text-slate-500 bg-white dark:bg-slate-900 rounded-3xl border border-stone-200/80 dark:border-slate-800 text-center">
          <Wrench className="h-10 w-10 text-stone-300 dark:text-slate-600 stroke-[1.5]" />
          <h3 className="text-sm font-bold text-stone-700 dark:text-slate-300">No Services Registered Yet</h3>
          ...
          <button
            onClick={openCreateModal}
            className="mt-2 flex items-center gap-1.5 rounded-2xl bg-amber-500 text-stone-950 px-4 py-2 text-xs font-bold shadow-2xs hover:bg-amber-400 transition cursor-pointer"
          >
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="font-mono font-bold text-xs text-stone-400 dark:text-slate-500">{srv.id.slice(0, 8)}</span>
                  <span className="rounded-full bg-emerald-50 dark:bg-emerald-950/60 px-2.5 py-0.5 text-[10px] font-bold text-emerald-700 dark:text-emerald-300 border border-emerald-100 dark:border-emerald-800">
                    ACTIVE
                  </span>
                </div>

                <span className="rounded-full bg-amber-50 dark:bg-amber-950/60 text-amber-900 dark:text-amber-300 px-2.5 py-0.5 text-[10px] font-bold border border-amber-100 dark:border-amber-800">
                  {srv.category?.name || "General Service"}
                </span>

                <h3 className="mt-3 text-base font-bold text-stone-900 dark:text-slate-100 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">
                  {srv.title}
                </h3>
                <p className="mt-1 text-xs text-stone-500 dark:text-slate-400 font-medium line-clamp-2">
                  {srv.description}
                </p>
              </div>

              <div className="mt-5 pt-3 border-t border-stone-100 dark:border-slate-800 flex items-center justify-between">
                <span className="text-base font-extrabold text-stone-900 dark:text-slate-100">${srv.price.toFixed(2)}</span>

                <div className="flex items-center gap-2">
                  <button
                    onClick={() => openEditModal(srv)}
                    className="flex h-8 w-8 items-center justify-center rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 text-stone-600 dark:text-slate-300 hover:bg-stone-900 dark:hover:bg-amber-500 hover:text-white dark:hover:text-slate-950 transition-colors cursor-pointer"
                  >
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-950/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-lg rounded-3xl bg-white dark:bg-slate-900 p-6 shadow-2xl space-y-4 border border-stone-200/80 dark:border-slate-800 text-stone-900 dark:text-slate-100">
            <div className="flex items-center justify-between border-b border-stone-100 dark:border-slate-800 pb-3">
              ...
              <button
                ...
                className="rounded-xl p-1 text-stone-400 dark:text-slate-400 hover:bg-stone-100 dark:hover:bg-slate-800 hover:text-stone-700 dark:hover:text-slate-200 cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            {errorMsg && (
              <div className="flex items-center gap-2 rounded-2xl bg-rose-50 dark:bg-rose-950/60 p-3 text-xs font-semibold text-rose-700 dark:text-rose-300 border border-rose-100 dark:border-rose-900">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 dark:text-slate-300 mb-1">Service Category</label>
                <select
                  value={categoryId}
              ...
              <div className="pt-2 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setModalOpen(false)}
                  className="rounded-2xl border border-stone-200 dark:border-slate-700 px-4 py-2 text-xs font-bold text-stone-700 dark:text-slate-300 hover:bg-stone-50 dark:hover:bg-slate-800 cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex items-center gap-1.5 rounded-2xl bg-stone-900 dark:bg-amber-500 dark:text-slate-950 px-5 py-2 text-xs font-bold text-white shadow-2xs hover:bg-stone-800 dark:hover:bg-amber-400 disabled:opacity-50 cursor-pointer"
                >
