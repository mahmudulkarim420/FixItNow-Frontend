...
export default function TechnicianJobsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "REQUESTED":
        return <span className="rounded-full bg-purple-50 dark:bg-purple-950/60 px-2.5 py-1 text-[10px] font-bold text-purple-700 dark:text-purple-300 border border-purple-100 dark:border-purple-800">Requested</span>;
      case "ACCEPTED":
        return <span className="rounded-full bg-blue-50 dark:bg-blue-950/60 px-2.5 py-1 text-[10px] font-bold text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-800">Accepted</span>;
      case "IN_PROGRESS":
        return <span className="rounded-full bg-amber-50 dark:bg-amber-950/60 px-2.5 py-1 text-[10px] font-bold text-amber-700 dark:text-amber-300 border border-amber-100 dark:border-amber-800">In Progress</span>;
      case "COMPLETED":
    ...
    <div className="space-y-6 text-stone-900 dark:text-slate-100">
      ...
      {loading ? (
        ...
        <div className="p-12 flex flex-col items-center justify-center gap-2 text-stone-400 dark:text-slate-500 bg-white dark:bg-slate-900 rounded-3xl border border-stone-200/80 dark:border-slate-800 text-center">
          <Briefcase className="h-10 w-10 text-stone-300 dark:text-slate-600 stroke-[1.5]" />
          <h3 className="text-sm font-bold text-stone-700 dark:text-slate-300">No Service Jobs Found</h3>
        ...
        <div className="space-y-6">
                ...
                >
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <span className="font-mono font-bold text-xs text-stone-400 dark:text-slate-500">{job.id.slice(0, 8)}</span>
                    ...
                    <div className="mt-3 space-y-1.5 text-xs text-stone-500 dark:text-slate-400 font-medium">
                      <p className="flex items-center gap-2">
                        <span className="font-bold text-stone-900 dark:text-slate-200">{job.customer?.name || "Customer"}</span> • {job.contactNumber || "N/A"}
                      </p>
                  ...
                  <div className="mt-5 pt-3 border-t border-stone-100 dark:border-slate-800 flex items-center justify-between">
                    <span className="text-base font-extrabold text-stone-900 dark:text-slate-100">${job.servicePrice}</span>
                    ...
                    <div className="flex items-center gap-2">
                      {job.status === "REQUESTED" && (
                        <button
                          ...
                          disabled={isUpdating}
                          className="flex items-center gap-1 rounded-xl bg-amber-500 text-stone-950 px-3 py-1.5 text-xs font-bold shadow-2xs hover:bg-amber-400 transition-colors disabled:opacity-50 cursor-pointer"
                        >
                      ...
                      {job.status === "ACCEPTED" && (
                        <button
                          ...
                          disabled={isUpdating}
                          className="flex items-center gap-1 rounded-xl bg-stone-900 dark:bg-amber-500 dark:text-slate-950 px-3 py-1.5 text-xs font-bold text-white shadow-2xs hover:bg-stone-800 dark:hover:bg-amber-400 transition-colors disabled:opacity-50 cursor-pointer"
                        >
                      ...
                      {job.status === "IN_PROGRESS" && (
                        <button
                          ...
                          disabled={isUpdating}
                          className="flex items-center gap-1 rounded-xl bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white shadow-2xs hover:bg-emerald-500 transition-colors disabled:opacity-50 cursor-pointer"
                        >
                      ...
                      <a
                        ...
                        className="flex items-center gap-1.5 rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-3 py-1.5 text-xs font-bold text-stone-700 dark:text-slate-300 hover:bg-stone-100 dark:hover:bg-slate-700 transition-colors"
                      >
          ...
          {totalItems > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 rounded-3xl border border-stone-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-2xs text-stone-900 dark:text-slate-100">
              <div className="flex items-center gap-3">
                <span className="text-xs font-semibold text-stone-500 dark:text-slate-400">
                  Showing <span className="font-bold text-stone-900 dark:text-slate-100">{startIndex + 1}</span> to{" "}
                  <span className="font-bold text-stone-900 dark:text-slate-100">{endIndex}</span> of{" "}
                  <span className="font-bold text-stone-900 dark:text-slate-100">{totalItems}</span> dispatches
                </span>

                <div className="flex items-center gap-1.5 pl-2 border-l border-stone-200 dark:border-slate-700">
                  <span className="text-[11px] font-bold text-stone-400 dark:text-slate-500">Per page:</span>
                  <select
                    value={itemsPerPage}
                    onChange={(e) => setItemsPerPage(Number(e.target.value))}
                    className="rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-2 py-1 text-xs font-bold text-stone-800 dark:text-slate-200 outline-none focus:border-amber-500"
                  >
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1 rounded-2xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-3 py-1.5 text-xs font-bold text-stone-700 dark:text-slate-300 hover:bg-stone-100 dark:hover:bg-slate-700 disabled:opacity-40 disabled:hover:bg-stone-50 transition-all cursor-pointer disabled:cursor-not-allowed"
                >
