...
import { DollarSign, Download, Calendar, TrendingUp, CheckCircle2, Clock, Loader2, FileSpreadsheet } from "lucide-react";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="rounded-2xl border border-stone-200/80 bg-white px-3 py-2 text-xs font-bold text-stone-700 outline-none focus:border-amber-500 shadow-2xs cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-2xl border border-stone-200 bg-white px-4 py-2 text-xs font-bold text-stone-700 shadow-2xs hover:bg-stone-50 transition-colors disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed"
          >
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="rounded-3xl bg-stone-900 p-5 text-white shadow-md flex flex-col justify-between">
              <div>
            ...
            <div className="rounded-3xl border border-stone-200/80 bg-white p-5 shadow-xs flex flex-col justify-between">
              <div>
                <span className="text-xs font-semibold text-stone-500">Pending Dispatches</span>
                <div className="mt-2 text-3xl font-extrabold text-stone-900">${pendingAmount.toFixed(2)}</div>
              </div>
            ...
            <div className="rounded-3xl border border-stone-200/80 bg-white p-5 shadow-xs flex flex-col justify-between">
              <div>
                <span className="text-xs font-semibold text-stone-500">Completed Revenue</span>
                <div className="mt-2 text-3xl font-extrabold text-stone-900">${totalGross.toFixed(2)}</div>
              </div>
          ...
          <div className="rounded-3xl border border-stone-200/80 bg-white shadow-xs overflow-hidden">
            <div className="px-5 py-4 border-b border-stone-100 flex items-center justify-between">
              <h3 className="text-sm font-bold text-stone-900">Settlement Payout Statements</h3>
            ...
            {statements.length === 0 ? (
              <div className="p-12 flex flex-col items-center justify-center gap-2 text-stone-400 text-center">
                <FileSpreadsheet className="h-10 w-10 text-stone-300 stroke-[1.5]" />
                <h4 className="text-sm font-bold text-stone-700">No Settlement Statements Yet</h4>
                  ...
                  <tbody className="divide-y divide-stone-100 font-medium text-stone-900">
                    {statements.map((e) => (
                      <tr key={e.id} className="hover:bg-stone-50/80 transition-colors">
                        ...
                        <td className="py-3.5 px-4 font-bold text-stone-900">{e.jobsCount} {e.jobsCount === 1 ? "Job" : "Jobs"}</td>
                        <td className="py-3.5 px-4 font-semibold text-stone-900">${e.gross.toFixed(2)}</td>
                        <td className="py-3.5 px-4 text-stone-400">-${e.fee.toFixed(2)}</td>
                        <td className="py-3.5 px-4 font-extrabold text-emerald-700">${e.net.toFixed(2)}</td>
