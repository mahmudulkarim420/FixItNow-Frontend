"use client";

import { Settings, Bell, Shield, Save } from "lucide-react";

export default function TechnicianSettingsPage() {
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-extrabold tracking-tight text-stone-900">
          Technician Settings
        </h1>
        <p className="mt-0.5 text-xs sm:text-sm font-medium text-stone-500">
          Manage job dispatch notifications, payout preferences, and account credentials.
        </p>
      </div>

      <div className="rounded-3xl border border-stone-200/80 bg-white p-5 sm:p-6 shadow-xs space-y-6">
        <div className="space-y-4">
          <h3 className="text-sm font-bold text-stone-900 flex items-center gap-2">
            <Bell className="h-4 w-4 text-amber-500" />
            <span>Dispatch Notifications</span>
          </h3>

          <div className="space-y-3">
            <label className="flex items-center justify-between p-3 rounded-2xl bg-stone-50 border border-stone-100 cursor-pointer">
              <span className="text-xs font-bold text-stone-900">SMS Job Dispatch Alerts</span>
              <input type="checkbox" defaultChecked className="h-4 w-4 accent-amber-500 rounded" />
            </label>
            <label className="flex items-center justify-between p-3 rounded-2xl bg-stone-50 border border-stone-100 cursor-pointer">
              <span className="text-xs font-bold text-stone-900">Email Booking Summaries</span>
              <input type="checkbox" defaultChecked className="h-4 w-4 accent-amber-500 rounded" />
            </label>
          </div>
        </div>

        <div className="pt-2 flex justify-end">
          <button className="flex items-center gap-2 rounded-2xl bg-stone-900 px-5 py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-stone-800 active:scale-95">
            <Save className="h-4 w-4 text-amber-400" />
            <span>Save Preferences</span>
          </button>
        </div>
      </div>
    </div>
  );
}
