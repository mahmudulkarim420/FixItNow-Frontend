...
import { Loader2, Users, Calendar, Star, DollarSign, Award, Wrench } from "lucide-react";
  ...
  useEffect(() => {
    async function loadAnalytics() {
      try {
        ...
        const colors = ["bg-amber-500", "bg-stone-900 dark:bg-amber-400", "bg-blue-500", "bg-emerald-500"];
        ...
        setCategoryBreakdown(
          breakdown.length > 0
            ...
            : [
                ...
                { name: "Plumbing Services", count: 10, percentage: 32, color: "bg-stone-900 dark:bg-amber-400" },
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="rounded-3xl border border-stone-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs flex flex-col justify-between">
              <div>
                <span className="text-xs font-semibold text-stone-500 dark:text-slate-400 flex items-center gap-1.5">
                  <Users className="h-3.5 w-3.5 text-amber-500" /> Registered System Users
                </span>
                <div className="mt-2 text-3xl font-extrabold text-stone-900 dark:text-slate-100">{metrics.activeUsers}</div>
              </div>
            ...
            <div className="rounded-3xl border border-stone-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs flex flex-col justify-between">
              <div>
                ...
                <div className="mt-2 text-3xl font-extrabold text-stone-900 dark:text-slate-100">{metrics.totalBookings}</div>
              </div>
            ...
            <div className="rounded-3xl border border-stone-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs flex flex-col justify-between">
              <div>
                ...
                <div className="mt-2 text-3xl font-extrabold text-stone-900 dark:text-slate-100">{metrics.totalRevenue}</div>
              </div>
              <span className="mt-3 inline-block rounded-full bg-emerald-50 dark:bg-emerald-950/60 px-2.5 py-0.5 text-[10px] font-bold text-emerald-700 dark:text-emerald-300 border border-emerald-100 dark:border-emerald-800 w-fit">
            ...
            <div className="rounded-3xl border border-stone-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs flex flex-col justify-between">
              <div>
                ...
                <div className="mt-2 text-3xl font-extrabold text-stone-900 dark:text-slate-100">{metrics.avgReviewScore}</div>
              </div>
              <span className="mt-3 inline-block rounded-full bg-amber-500/10 dark:bg-amber-950/60 px-2.5 py-0.5 text-[10px] font-bold text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800 w-fit">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="rounded-3xl border border-stone-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-2xs flex flex-col justify-between">
              <div>
                <h3 className="text-sm font-bold text-stone-900 dark:text-slate-100 mb-4 flex items-center gap-2">
                  <Wrench className="h-4 w-4 text-amber-500" /> Service Demand Distribution
                </h3>
                <div className="space-y-4">
                  {categoryBreakdown.map((cat) => (
                    <div key={cat.name}>
                      <div className="flex justify-between text-xs font-bold text-stone-900 dark:text-slate-100 mb-1">
                        <span>{cat.name}</span>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {topTechs.map((tech, idx) => (
                    <div
                      key={tech.name}
                      className="flex items-center justify-between p-3 rounded-2xl bg-stone-50 dark:bg-slate-800/70 border border-stone-100 dark:border-slate-800"
                    >
                      <div className="flex items-center gap-3">
                        <span className={`font-extrabold text-sm ${idx === 0 ? "text-amber-500" : "text-stone-400 dark:text-slate-500"}`}>
                          #{idx + 1}
                        </span>
                        <div>
                          <h4 className="text-xs font-bold text-stone-900 dark:text-slate-100">{tech.name}</h4>
                          <p className="text-[10px] text-stone-400 dark:text-slate-400 font-medium">
