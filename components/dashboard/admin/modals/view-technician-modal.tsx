...
import { X, ShieldCheck, AlertTriangle, Mail, Star, Wrench, DollarSign, Award, CheckCircle2 } from "lucide-react";
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-xl text-stone-400 hover:bg-stone-100 hover:text-stone-700 transition-colors cursor-pointer"
          >
            <X className="h-5 w-5" />
          </button>
          ...
          <div className="flex flex-col items-center text-center pb-4 border-b border-stone-100">
            <div className="relative mb-3">
              ...
              <img
                src={tech.avatar}
                alt={tech.name}
                className="h-20 w-20 rounded-3xl object-cover ring-4 ring-amber-100 shadow-md"
              />
              <span
                className={`absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full border-2 border-white shadow-xs ${
            ...
            <div className="flex items-center gap-1.5 mt-0.5 text-xs text-stone-500 font-medium">
          ...
          <div className="space-y-2 mb-5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {tech.skills.map((s) => (
                <span
                  key={s}
                  className="inline-flex items-center gap-1 rounded-2xl bg-amber-50 px-3 py-1 text-xs font-bold text-amber-800 border border-amber-100"
                >
                  <Wrench className="h-3 w-3 text-amber-600" /> {s}
                </span>
              ))}
            </div>
          </div>
