...
import { Search, Mail, Bell, Menu, ShieldCheck } from "lucide-react";
    ...
    <header className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 sm:gap-4 py-2">
      ...
      <div className="flex items-center gap-2.5 flex-1 w-full">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Search job #, address, customer..."
      ...
      {/* Right Controls: Notifications, Messages, Technician User Info */}
        ...
        <div className="flex items-center gap-2.5 rounded-2xl border border-stone-200/80 bg-white p-1.5 pr-3.5 shadow-xs transition-all hover:border-stone-300">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-1">
              <span className="text-xs font-bold text-stone-900 leading-tight truncate max-w-[100px] sm:max-w-[130px]">
                {user.name}
              </span>
              <ShieldCheck className="h-3.5 w-3.5 text-emerald-500 shrink-0" />
            </div>
            <span className="hidden sm:block text-[10px] font-medium text-amber-700">
              Verified Field Tech
            </span>
          </div>
        </div>
